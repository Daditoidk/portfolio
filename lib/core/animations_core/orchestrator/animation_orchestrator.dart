...
class AnimationOrchestrator extends ChangeNotifier {
  ...
  void registerAnimation(String id, AnimationInstanceState animationState) {
    if (_animations.containsKey(id)) {
      _addGlobalWarning('Animation $id already registered, updating state');
    }

    _animations[id] = animationState;
    _addGlobalInfo('Registered animation: $id (${animationState.type})');
    notifyListeners();
  }
